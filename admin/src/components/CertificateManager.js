...
const CertificateManager = () => {
    ...
    const fetchStudents = async () => {
        try {
            const response = await axios.get(`${API_BASE}/api/wallets/list`);
    ...
    const handleMintDiploma = async () => {
        if (!selectedStudent) {
            setMessage({ type: 'error', text: 'Оберіть студента' });
            return;
        }
        ...
        try {
            const result = await mintDiploma(selectedStudent.address, metadataURI, diplomaData, imageFile);
            setMessage({
                type: 'success',
                ...
                <Grid item xs={12} md={6}>
                    <Card>
                        <CardContent>
                            ...
                            <Autocomplete
                                options={students}
                                getOptionLabel={(option) => `${option.username} (${option.address.substring(0, 6)}...)`}
                                value={selectedStudent}
                                onChange={(e, newValue) => {
                                    setSelectedStudent(newValue);
                                    setStudentName(newValue?.username || '');
                                }}
                                renderInput={(params) => (
                                    <TextField {...params} label="Студент" margin="normal" size="small" />
                                )}
                                renderOption={(props, option) => (
                                    <li {...props}>
                                        ...
                                        {option.username} <Chip size="small" label={option.address.substring(0, 6) + '...'} sx={{ ml: 1 }} />
                                    </li>
                                )}
                            />
